...
public class Project {
     ...
     *
     * questa procedura "divide" l'array in tre parti:
     * 1) elementi minori dell'elemento pivot
     * 2) elementi uguali all'elemento pivot
    ...
    public static int partition(double[] array, int left, int right, int p) {
        ...
        int j = left;
        int n = right;
        while (j <= n) { //finchè j <= numero degli elementi rimasti da "mettere a posto"
            if (array[j] < array[p]) {
                swap(array, i, j); //se l'elemento all'indice j è minore del pivot scambialo con l'elemento in i
                i++;
                j++;
            } else if (array[j] > array[p]) { //inizialmente n sarà l'ultimo indice dell'array
                                             //da quella parte vanno gli elementi maggiori di p
                swap(array, j, n); //se invece l'elemento è maggiore scambialo con l'elemento in posizione n
                n = n - 1; //decrementa n
            } else {
                j = j + 1;
            }
