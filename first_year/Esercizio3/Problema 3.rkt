...
(define stringcreate; Crea la stringa unendo il valore della parte destra con quello della parte sinistra
  (lambda (str cifre) ; str,cifre: Stringhe
    (if (= (string-length str) (findot str 0))
        ...
        (string-append (number->string (rep->dec (substring str 0 (findot str 0)) 0 cifre)) ;passa rep->dec la parte prima del punto
         "." (number->string (rep->dec (substring str (findot str 1) (string-length str)) 1 cifre)) ;passa a rep->dec la parte dopo il punto 
         ))
    ))
...
(define rep->dec
  (lambda (str sgn cifre) ; sgn: intero (0 = sinistra) (1 o altro = destra) str,cifre: Stringhe
    (if (= sgn 0)
        (convs1 str 0 cifre) ;se il segno è uguale a 0 converti la parte a sinistra, n è il numero che alla fine si restituisce
   ...
   (let ((remove1 (if (> (string-length str) 0)(substring str 1)""))
    ...
    (if (> (string-length str) 0)
     (if (= pesochar 0)
      ...
      (convd1 remove1 (+ (* (expt (/ 1 base) (+ c 1)) pesochar) n) cifre c)  ;la stessa cosa di convd solo che al posto si fa la potenza della parte frazionaria
      )
     n
     )
   )))
