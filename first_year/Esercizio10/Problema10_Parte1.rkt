  ...
  (lambda(rot)
    (lambda(c lst) ;c: primo carattere della stringa "ciaomondo"
      ...
      (let( (p (+ (numera lst c 0) rot)))
        ...
        (if(> p 19)
           (converti (- p 20) lst 0)
           (converti p lst 0) ;lst: lista dell'alfabeto latino
           )
        )
      )
    )
...
(define elimina ;restituisce la stringa a cui vengono rimossi eventuali spazi
  (lambda(msg)
    ...
    (if (= (conta msg) 0) ;conto ogni volta gli spazi finchè non sono uguali a 0, a quel punto ritorno il messaggio
        msg
...
(define converti
  (lambda(pos lst i) ;pos: valore numerico della posizione lista: lista dell'alfabeto latino
    ...
    (if(= pos i) ;controllo che la poszione sia uguale all'indice (che inizialmente è uguale a 0)
       (list-ref lst i) ;ritorna il valore della lista corrispondente al valore in posizione i
       (converti pos lst (+ i 1)) ;se non è uguale lo incremento fino a che non è uguale, a quel punto restituisco l'elemento della lista (dell'alfabeto latino latino) corrispondente all'indice(
       ) 
...
(crit-cesare "CIAO MI CHIAMO MAESTRO E MI PIACE GIOCARE A BASKET" (regola 4))